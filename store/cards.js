import { defineStore } from "pinia";

export const useCardStore = defineStore("cards", {
  state: () => {
    return {
      deck: [
        {
          name: "Ás de Espadas",
          suit: "espadas",
          letter: "A",
        },
        {
          name: "Ás de Paus",
          suit: "paus",
          letter: "A",
        },
        {
          name: "Ás de Copas",
          suit: "copas",
          letter: "A",
        },
        {
          name: "Ás de Ouros",
          suit: "ouros",
          letter: "A",
        },
        {
          name: "Dois de Espadas",
          suit: "espadas",
          letter: "2",
        },
        {
          name: "Dois de Paus",
          suit: "paus",
          letter: "2",
        },
        {
          name: "Dois de Copas",
          suit: "copas",
          letter: "2",
        },
        {
          name: "Dois de Ouros",
          suit: "ouros",
          letter: "2",
        },
        {
          name: "Três de Espadas",
          suit: "espadas",
          letter: "3",
        },
        {
          name: "Três de Paus",
          suit: "paus",
          letter: "3",
        },
        {
          name: "Três de Copas",
          suit: "copas",
          letter: "3",
        },
        {
          name: "Três de Ouros",
          suit: "ouros",
          letter: "3",
        },
        {
          name: "Quatro de Espadas",
          suit: "espadas",
          letter: "4",
        },
        {
          name: "Quatro de Paus",
          suit: "paus",
          letter: "4",
        },
        {
          name: "Quatro de Copas",
          suit: "copas",
          letter: "4",
        },
        {
          name: "Quatro de Ouros",
          suit: "ouros",
          letter: "4",
        },
        {
          name: "Cinco de Espadas",
          suit: "espadas",
          letter: "5",
        },
        {
          name: "Cinco de Paus",
          suit: "paus",
          letter: "5",
        },
        {
          name: "Cinco de Copas",
          suit: "copas",
          letter: "5",
        },
        {
          name: "Cinco de Ouros",
          suit: "ouros",
          letter: "5",
        },
        {
          name: "Seis de Espadas",
          suit: "espadas",
          letter: "6",
        },
        {
          name: "Seis de Paus",
          suit: "paus",
          letter: "6",
        },
        {
          name: "Seis de Copas",
          suit: "copas",
          letter: "6",
        },
        {
          name: "Seis de Ouros",
          suit: "ouros",
          letter: "6",
        },
        {
          name: "Sete de Espadas",
          suit: "espadas",
          letter: "7",
        },
        {
          name: "Sete de Paus",
          suit: "paus",
          letter: "7",
        },
        {
          name: "Sete de Copas",
          suit: "copas",
          letter: "7",
        },
        {
          name: "Sete de Ouros",
          suit: "ouros",
          letter: "7",
        },
        {
          name: "Oito de Espadas",
          suit: "espadas",
          letter: "8",
        },
        {
          name: "Oito de Paus",
          suit: "paus",
          letter: "8",
        },
        {
          name: "Oito de Copas",
          suit: "copas",
          letter: "8",
        },
        {
          name: "Oito de Ouros",
          suit: "ouros",
          letter: "8",
        },
        {
          name: "Nove de Espadas",
          suit: "espadas",
          letter: "9",
        },
        {
          name: "Nove de Paus",
          suit: "paus",
          letter: "9",
        },
        {
          name: "Nove de Copas",
          suit: "copas",
          letter: "9",
        },
        {
          name: "Nove de Ouros",
          suit: "ouros",
          letter: "9",
        },
        {
          name: "Dez de Espadas",
          suit: "espadas",
          letter: "10",
        },
        {
          name: "Dez de Paus",
          suit: "paus",
          letter: "10",
        },
        {
          name: "Dez de Copas",
          suit: "copas",
          letter: "10",
        },
        {
          name: "Dez de Ouros",
          suit: "ouros",
          letter: "10",
        },
        {
          name: "Valete de Espadas",
          suit: "espadas",
          letter: "J",
        },
        {
          name: "Valete de Paus",
          suit: "paus",
          letter: "J",
        },
        {
          name: "Valete de Copas",
          suit: "copas",
          letter: "J",
        },
        {
          name: "Valete de Ouros",
          suit: "ouros",
          letter: "J",
        },
        {
          name: "Dama de Espadas",
          suit: "espadas",
          letter: "Q",
        },
        {
          name: "Dama de Paus",
          suit: "paus",
          letter: "Q",
        },
        {
          name: "Dama de Copas",
          suit: "copas",
          letter: "Q",
        },
        {
          name: "Dama de Ouros",
          suit: "ouros",
          letter: "Q",
        },
        {
          name: "Rei de Espadas",
          suit: "espadas",
          letter: "K",
        },
        {
          name: "Rei de Paus",
          suit: "paus",
          letter: "K",
        },
        {
          name: "Rei de Copas",
          suit: "copas",
          letter: "K",
        },
        {
          name: "Rei de Ouros",
          suit: "ouros",
          letter: "K",
        },
      ],
      drawableCards: [],
      playedCards: [],
    };
  },

  actions: {
    resetDeck() {
      this.drawableCards = this.deck;
      this.playedCards = [];
    },

    drawCard() {
      if (this.drawableCards.length > 0) {
        // selects a position on the drawable cards array
        const cardPosition = Math.floor(Math.random()*this.drawableCards.length);

        // gets the card from that position and removes it from the array
        const randomCard = this.drawableCards[cardPosition];
        this.drawableCards.splice(cardPosition, 1);
        
        // adds the selected card to the beginning of the played cards array
        this.playedCards.reverse();
        this.playedCards.push(randomCard);
        this.playedCards.reverse();
      }
    },
  },

  getters: {
    getDrawableCards: (state) => state.drawableCards,
    getPlayedCards: (state) => state.playedCards,
  }
});